...
func ApplyCLI() func(cliCtx *cli.Context) error {
	return func(cliCtx *cli.Context) error {
		...
		cleanupAfterExit := cliCtx.Bool(CleanupAfterExitFlagName)
...
func ApplyPipeline(
	...
	l1ArtifactsFS, cleanupL1Artifacts, err := artifacts.Download(ctx, intent.L1ContractsLocator, artifacts.BarProgressor())
	if opts.CleanupAfterExit && cleanupL1Artifacts != nil {
		defer func() {
			if err := cleanupL1Artifacts(); err != nil {
				opts.Logger.Error("failed to cleanup L1 artifacts", "error", err)
			}
		}()
	}
	if err != nil {
	...
	if intent.L1ContractsLocator.Equal(intent.L2ContractsLocator) {
		...
		l2Afs, cleanupL2Artifacts, err := artifacts.Download(ctx, intent.L2ContractsLocator, artifacts.BarProgressor())
		if opts.CleanupAfterExit && cleanupL2Artifacts != nil {
			defer func() {
				if err := cleanupL2Artifacts(); err != nil {
					opts.Logger.Error("failed to cleanup L2 artifacts", "error", err)
				}
			}()
		}
		if err != nil {
